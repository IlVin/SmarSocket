...
uint8\_t [Id]       - Идентификатор пакета
uint8\_t [Data\*]    - Байты данных
uint8\_t [CheckSum] - Контрольная сумма
```
...
Для передачи показаний АЦП используются пакеты вида ` [Id][DataHi][DataLo] `. Вся телеметрия
...
```
int16\_t VALUE = static\_cast<int16\_t>( (static\_cast<uint16\_t>([DataHi]) << 7) | static\_cast<uint16\_t>([DataLo]) )
```
